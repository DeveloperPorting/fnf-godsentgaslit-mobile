...
name_of_character_xml = 'catnap2'
name_of_character = 'catnap2'
charNote = 'catnap2'
...
function onCreate()
	...
	setProperty(name_of_character .. '.flipX', flipX)
	setProperty(name_of_character .. '.alpha', not invisible)
	scaleObject(name_of_character, xScale, yScale)
	setObjectOrder(name_of_character, layer, false)
	setObjectOrder('catnap2', 8)
	setScrollFactor('catnap2', 0, 0)
	setProperty('catnap2.alpha', 0)
end
...
function idleDance(beat)
	if useIdle then
		if beat % 2 == 0 then
			playAnim(name_of_character, 'idle', false)
			setProperty(name_of_character .. '.offset.x', idleOffsets[1])
			setProperty(name_of_character .. '.offset.y', idleOffsets[2])
		end
	else
		if beat % 2 == 0 then
			playAnim(name_of_character, 'danceLeft', false)
			setProperty(name_of_character .. '.offset.x', idleOffsets[1])
			setProperty(name_of_character .. '.offset.y', idleOffsets[2])
		else
			playAnim(name_of_character, 'danceRight', false)
			setProperty(name_of_character .. '.offset.x', idleOffsets[1])
			setProperty(name_of_character .. '.offset.y', idleOffsets[2])
		end
	end
